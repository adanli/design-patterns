...
## 中介者模式=调停者模式(<i>Mediator</i>)
### 使用场景
- 遵循迪米特原则，使各个对象在没有强关联的情况下，它们之间的交互不需要直接耦合
- 聊天室(erpc的server端?)

## 享元模式(<i>Flyweight</i>)
### 使用场景
- 当需要大量的实例对象来表现数据时，如果这些实例除了几个参数以外其他基本相同的场景
### 实现方式
- 使用一个HashMap来存储对象，以保证类似的对象能够被复用

## 访问者模式(<i>Visitor</i>)
### 使用场景
- 将数据结构和作用于数据结构上的操作解耦
- 当需要去新增操作方法的时候，只需要添加`visitor`里的方法即可，不需要修改已有的对象的数据结构